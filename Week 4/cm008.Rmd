...
<!-- group by is redundant here because ggplot doesn't use it. Need to specify group in ggplot -->

```{r, fig.width = 5, fig.height = 2}
gapminder %>% 
  #group_by(country) %>% 
  ggplot(aes(year, lifeExp, group = country, colour = country == "Rwanda")) + #color only Rwanda
  geom_line(alpha = 1/3) +
  scale_color_discrete("", labels = c("Other", "Rwanda"))+ #Change the legend labels and remove the legend title
  theme_bw()
```
...
ggplot(gapminder, aes(gdpPercap, lifeExp)) +
  ...
  facet_wrap(~continent, scales = "free" ) #scales = free gives each panel its own x and y axis but can no longer compare between plots because the scales are different. Can also do "free_y"
```
...
gapminder %>% 
  filter(continent != "Oceania") %>% #"!=" filters out Oceania
