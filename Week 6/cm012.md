...
```r
h_countries <-  c( "Australia","Canada","Japan", "Egypt", "Cambodia")
gap <- gapminder %>%
...
gapminder %>%
  ggplot() +
  geom_bar(aes(fct_reorder(continent, lifeExp, min))) + #sort by minimum life exp for each factor (default state is median)
  coord_flip()+
  theme_bw() +
...
This might be useful if you are preparing a report for say, the state of affairs in Oceania. (fct_relevel)
