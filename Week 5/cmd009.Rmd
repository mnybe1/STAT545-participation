...
Yes - although tidy data is relative
...
```{r}
lotr_wide %>% 
  pivot_longer(cols  = c(-Film, -Gender), #Could do "Elf, Hobbit, Man" or this
               names_to  =  "Race", 
               values_to = "words")
```
...
<!--what side of the column name is going to retain its' column status (if any) -->
...
```{r}
guest_long %>% 
  separate(name, into = c("first", "last"), sep = " ") %>% #sep = " " because some last names have hyphens
  unite(col = "name", first, last, sep = " ")
```

3. Which parties still have a "PENDING" status for all members and all events?
<!-- a pending for everyone in a party and for all events --> 
...
#there is a new column that is meal_gold so use dplyr to drop it
```
...
experiment %>% complete(nesting(name, trt), rep) #groups name and trt together 
```
